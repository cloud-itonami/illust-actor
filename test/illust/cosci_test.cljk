(ns illust.cosci-test
  (:require [clojure.test :refer [deftest testing is]]
            [illust.cosci :as cosci]))

(defn- result [id status & {:keys [format bytes safety] :or {format "png" bytes (byte-array [1]) safety false}}]
  {:candidate {:candidate/id id :prompt (str "prompt-" id) :gene {:subject "s" :style "st"}}
   :status status :artifact-bytes (if (= status :done) bytes nil)
   :format (when (= status :done) format) :expected-format "png" :safety-flag safety})
...
(deftest reflect-hard-gates
  ...
  (testing "format mismatch fails"
    (is (false? (:pass? (cosci/reflect (result "a" :done :format "psd"))))))
...
(deftest run-round-evolution-advances-round
  (let [{:keys [evolution]} (cosci/run-round [(result "a" :done)] {"a" {:score 80}} 5)]
    ...
    (is (= {:subject "s" :style "st"} (:elite-gene evolution)))))
