(ns illust.loop
  ...
  through illust.cosci -> illust.governor -> commit+publish. `run!` reduces
  ...
  queue — see illust.murakumo's docstring."
  (:require [clojure.java.io :as io]
            [clojure.edn :as edn]
            [kotoba.lang.text :as str]
            [illust.persona :as persona]
            [illust.generate :as generate]
            [illust.murakumo :as murakumo]
            [illust.judge :as judge]
            [illust.cosci :as cosci]
            [illust.governor :as governor]
            [illust.datalad :as datalad]
            [illust.ledger :as ledger]
            [illust.publish :as publish]
            [illust.aozora :as aozora]))

(def lease-path ".illust/lease")
(def state-path ".illust/state.edn")
...
(defn- fetch-result
  "One pending entry -> a illust.cosci/reflect-shaped result map, or nil if
...
(defn- asset-title [gene]
  (str/join ", " (remove nil? [(:subject gene) (:style gene)])))
...
(defn settle-round!
  ...
  illust.publisher/mock-publisher in tests."
   ...
   (let [p (persona/load-persona)
     ...
     (when winner
       (let [{:keys [result score]} winner
             ...
             asset {:id (str "illust-" (str/replace job-id #"[^a-zA-Z0-9-]" "-"))
