(ns illust.judge
  "JVM I/O edge: score a candidate for persona-fit/creative quality via the
  ...
  cloud_itonami.media's own docstring): this judges the CANDIDATE'S PROMPT
...
(defn score!
  ...
  failure (gateway unreachable, non-200, unparseable) — illust.cosci/reflect
  ...
  ([{:keys [prompt]} {:keys [voice tags]}
   ...
   (try
     (let [url (or url (gateway-url))
           sys (str "You are " voice " Rate the following image-generation prompt "
                    "for creative quality and fit to your own tags ("
