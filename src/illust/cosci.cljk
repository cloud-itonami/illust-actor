(ns illust.cosci
  "Google co-scientist / AlphaEvolve-shaped tournament over one round of image
  candidates (ADR-2607123000 §3), ported-in-shape from
  ...
  slightly-worse asset, it's a disqualified one.
    ...
    Generation  -> illust.generate/round-candidates (closed gene pool)
    Reflection  -> hard gates: job actually :done, artifact fetched,
                   declared format matches, no upstream safety flag
    Ranking     -> Elo on illust.judge scores (pairwise, draw-tolerant)
...
(defn reflect
  ...
  :format \"png\" :expected-format \"png\" :safety-flag bool}."
...
(defn rank
  ...
  missing entries when illust.judge/score! failed) -> ranked vector,
...
(defn evolve-round
  "ranked -> {:next-round n+1 :elite-gene {...}} for illust.generate's next
